...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      vim.list_extend(ensure_installed, {
        ...
        'remark-cli', -- Markdown linter/formatter
      })
  ...
  { -- Highlight, edit, and navigate code
    ...
    branch = 'main', -- The `main` branch is the supported one for Neovim 0.11+; `master` is frozen and breaks on 0.12.
    build = ':TSUpdate',
    -- [[ Configure Treesitter ]] See `:help nvim-treesitter`
    -- The `main` branch dropped the old `nvim-treesitter.configs` module + opts table.
    -- Setup is now: install parsers, then enable highlight/indent via a FileType autocmd.
    config = function()
      ...
      vim.api.nvim_create_autocmd('FileType', {
        ...
        callback = function(args)
          ...
          -- Start highlighting only if the parser is actually available.
          if pcall(vim.treesitter.start, buf, lang) then
            -- Treesitter-based indentation (experimental). Ruby relies on vim's
            -- own regex indent rules, so leave its indentexpr untouched.
            if ft ~= 'ruby' then
